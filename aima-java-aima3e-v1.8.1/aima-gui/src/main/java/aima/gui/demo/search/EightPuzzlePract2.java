...
public class EightPuzzlePract2 {
	...
	public static void pruebas() {
		try {
			//Estado inicial, 3 pasos solución óptima
			EightPuzzleBoard boardWithThreeMoveSolution = new EightPuzzleBoard(
				new int[] { 1, 2, 5, 3, 4, 0, 6, 7, 8 });
			int expandedNodes, generatedNodes;
			...
			if(agent.getInstrumentation().getProperty("nodesGenerated") == null) generatedNodes = 0; // NODOS GENERADOS
			else generatedNodes = (int)Float.parseFloat(agent.getInstrumentation().getProperty("nodesGenerated"));
			
			Biseccion b = new Biseccion();
			b.setDepth(5);
			b.setGeneratedNodes(generatedNodes);
			double fr_efectivo = b.metodoDeBiseccion(1.001, 4.0, 0.1);
			
			System.out.format("BFS %15s|%11s|%11s\n",expandedNodes, generatedNodes, fr_efectivo);
			...
			//IDS--------------------------
			Search search2 = new IterativeDeepeningSearch();
			SearchAgent agent2 = new SearchAgent(problem, search2);
			if(agent2.getInstrumentation().getProperty("nodesExpanded") == null) expandedNodes = 0;
			else expandedNodes = (int)Float.parseFloat(agent2.getInstrumentation().getProperty("nodesExpanded"));
			
			if(agent2.getInstrumentation().getProperty("nodesGenerated") == null) generatedNodes = 0; // NODOS GENERADOS
			else generatedNodes = (int)Float.parseFloat(agent2.getInstrumentation().getProperty("nodesGenerated"));
			b.setDepth(9);
			b.setGeneratedNodes(generatedNodes);
			fr_efectivo = b.metodoDeBiseccion(1.000000000001, 4.0, 1E-12);
			
			System.out.format("IDS %15s|%11s|%11s\n",expandedNodes, generatedNodes, fr_efectivo);

			
			//A*---------------------------
			Search search3 = new AStarSearch(new GraphSearch(),
					new MisplacedTilleHeuristicFunction());
			SearchAgent agent3 = new SearchAgent(problem, search3);
			
			if(agent3.getInstrumentation().getProperty("nodesExpanded") == null) expandedNodes = 0;
			else expandedNodes = (int)Float.parseFloat(agent3.getInstrumentation().getProperty("nodesExpanded"));
			
			if(agent3.getInstrumentation().getProperty("nodesGenerated") == null) generatedNodes = 0; // NODOS GENERADOS
			else generatedNodes = (int)Float.parseFloat(agent3.getInstrumentation().getProperty("nodesGenerated"));
			b.setDepth(3);
			b.setGeneratedNodes(generatedNodes);
			fr_efectivo = b.metodoDeBiseccion(1.000000000001, 4.0, 1E-12);
			
			System.out.format("A* %15s|%11s|%11s\n",expandedNodes, generatedNodes, fr_efectivo);
			...
			//Estado final aleatorio a profundidad n
			int depthOK = 22; int depth;
			EightPuzzleBoard end = new EightPuzzleBoard(GenerateInitialEightPuzzleBoard.random(depthOK, init));
			System.out.println("end ->\n" + end + "\n");
			
			//Prueba heurística MisplacedTilleHeuristicFunction2 || Manhat
			
			EightPuzzleGoalTest test = new EightPuzzleGoalTest(end);
			System.out.println("end ->\n" + EightPuzzleGoalTest.getGoal() + "\n");
			
			//MisplacedTilleHeuristicFunction2 h = new MisplacedTilleHeuristicFunction2();
			ManhattanHeuristicFunction2 h = new ManhattanHeuristicFunction2();
			
			Problem problem4 = new Problem(init, EightPuzzleFunctionFactory
					.getActionsFunction(),EightPuzzleFunctionFactory
					.getResultFunction(), test);
			
			
			Search search4 =new AStarSearch(new GraphSearch(),h);
		 
			SearchAgent agent4 = new SearchAgent(problem4, search4);
			
			String pathcostM = agent4.getInstrumentation().getProperty("pathCost");
			if (pathcostM!=null) depth = (int)Float.parseFloat(pathcostM);
			else depth = 0;

			System.out.println("profundidad -> " + depthOK);
			
			System.out.println("camino A* -> " + depth);			
